...
if __name__ == '__main__':
    assert getTopRatedFoodOutlets('Denver') == ['Sushi Kashiba', 'Peninsula Grand Hotel',
                                                'Plum by Bent Chair', "R' ADDA",
                                                'Palladium Social']
    assert getTopRatedFoodOutlets('Houston') == ['Barbeque Nation']
    assert getTopRatedFoodOutlets('Seattle') == ['Cafe Juanita', 'Cafe Munir',
                                                 'Asia Kitchen By Mainland China',
                                                 "AB's - Absolute Barbecues"]
    assert getTopRatedFoodOutlets('Miami') == ['Decode Air Bar']
    assert getTopRatedFoodOutlets(
        'Omaha') == ['The Saffron Tree', 'Echoes Omaha', 'Barbeque Nation']
    assert getTopRatedFoodOutlets('Dallas') == ['Coal Barbecues', 'Coal Barbecues', "AB's - Absolute Barbecues",
                                                "AB's - Absolute Barbecues", "AB's - Absolute Barbecues", "AB's - Absolute Barbecues", 'Bateau']
